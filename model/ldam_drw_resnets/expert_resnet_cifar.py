...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_experts, num_classes=10, reduce_dimension=False, layer2_output_dim=None, layer3_output_dim=None, use_norm=False, returns_feat=True, use_experts=None, s=30,head="mlp"):
        ...
        if head == 'mlp':
            self.head = nn.ModuleList([nn.Sequential(nn.Linear(layer3_output_dim, 512), nn.BatchNorm1d(512), nn.ReLU(inplace=True),
                                      nn.Linear(512, 128)) for _ in range(num_experts)])
        else:
            raise NotImplementedError(
                'head not supported'
            )

        self.head_fc = nn.ModuleList([nn.Sequential(nn.Linear(layer3_output_dim, 512), nn.BatchNorm1d(512), nn.ReLU(inplace=True),
                                     nn.Linear(512, 128)) for _ in range(num_experts)] )
    ...
    def forward(self, x):
        ...
        for ind in use_experts:
            ...
            if self.use_norm:
              centers_logits.append(F.normalize(self.head_fc[ind](self.linears[ind].weight.T)))
            else:
              centers_logits.append(F.normalize(self.head_fc[ind](self.linears[ind].weight)))
            feat_mlp.append(F.normalize(self.head[ind](self.feat[ind]),dim=1))

        final_out = torch.stack(outs, dim=1).mean(dim=1)
        self.feat = torch.stack(self.feat, dim=1)
        self.feat_before_GAP = torch.stack(self.feat_before_GAP, dim=1)
        feat_mlp = torch.stack(feat_mlp,dim=1).mean(dim=1)
        centers_logits = torch.stack(centers_logits,dim=1).mean(dim=1)
