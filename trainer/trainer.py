        ...
        for batch_idx, data in enumerate(self.data_loader):
            ...
            with autocast():
                if self.real_model.requires_target:
                    ...
                    centers = output["centers_logits"]
                    centers = centers[:100]
                    _, f2, f3 = torch.split(output["feat_mlp"], [batch_size, batch_size, batch_size], dim=0)
                    features = torch.cat([f2.unsqueeze(1), f3.unsqueeze(1)], dim=1)
          
                    logits, _, __ = torch.split(output["logits"], [batch_size, batch_size, batch_size], dim=0)
                    ...
                    if self.add_extra_info:
                        ce_loss = self.criterion(output_logits=output, target=target, extra_info=extra_info)
                    else:
                        ce_loss = self.criterion(output_logits=output, target=target)

                    criterion_scl = BalSCL(self.data_loader.cls_num_list, 0.1).cuda()
                    scl_loss = criterion_scl(centers, features, target)

                    loss = 2*ce_loss + 0.6*scl_loss
